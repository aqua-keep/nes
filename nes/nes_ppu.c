...
uint16_t line_colors[256]; // 图像缓存

void set_tile_banks(uint8_t* bank0, uint8_t* bank1, uint8_t* bank2, uint8_t* bank3, uint8_t* bank4, uint8_t* bank5,
                    uint8_t* bank6, uint8_t* bank7)
{
...
uint32_t spr_clip_left8()
{
    return !(ppu->LowRegs[1] & 0x04);
}

uint32_t bg_clip_left8()
{
...
// 0x2002
uint32_t sprite0_hit()
{
    return ppu->LowRegs[2] & 0x40;
}

uint32_t more_than_8_sprites_on_cur_line()
{
    return ppu->LowRegs[2] & 0x20;
}

uint32_t VRAM_accessible()
{
...
/* scanline start (if background or sprites are enabled): v:0000010000011111=t:0000010000011111 */
...
void PPU_reset(void)
{
    ...
    if (RomHeader->flags_1 & 0x01)
        set_mirroring(0, 1, 0, 1); // 垂直镜像
        // PPU_set_mirroring(); 设置镜像
    else
...
void do_scanline_and_dont_draw(int LineNo)
{
    ...
    // else
    {
        ...
        if ((!sprite0_hit()) && (ppu->current_frame_line >= ((uint32_t)(spr_ram[0] + 1))) && (ppu->current_frame_line <
...
void render_bg(uint8_t* buf)
{
    ...
    for (i = 33; i; i--)
    {
        pattern_addr = ppu->bg_pattern_table_addr + ((uint32_t)VRAM(name_addr) << 4) + line;
...
{
    uint32_t s; // sprite #
    uint32_t spr_x; // sprite coordinates
    uint32_t spr_y;
    ...
    uint32_t priority;
    uint32_t tile_addr;
    uint32_t inc_x; // drawing vars
    uint32_t start_x, end_x;
    uint32_t x, y; // in-sprite coords
    uint32_t col;
    ...
    for (s = 0, spr = spr_ram; s < 64; s++, spr += 4)
    {
        ...
        {
            inc_x = -1;
            // 此处保留原逻辑的翻转循环参数，但指针 p 已在上方正确初始化，不应再移动
            start_x = (8 - 1) - start_x;
            end_x = (8 - 1) - end_x;
        }
        else
...
#else
                // 原代码: col = pattern >> ((7 - (x & 7)) * 2); 导致翻转时像素提取顺序错误
                // 修正后: 直接取当前 x 索引对应的低位数据，配合循环的方向即可实现翻转
                col = pattern >> ((7 - (x & 7)) * 2);
#endif
                col &= 0x03;
                if (col)
